...
class AntColonyRCPSP:
    ...
    def _schedule_makespan(self, task_order: List[Task]) -> int:
        """
        Calculate the makespan considering resource constraints and parallel execution.
        """
        # Initialize resource availability over time
        max_time = 1000  # Large enough to cover all possible schedules
        ...
        for task in task_order:
            if task.name == '1':  # Skip dummy start
                continue
                
            # Find earliest possible start time considering:
            # 1. Predecessor completion times
            # 2. Resource availability
            earliest_start = 0
            for pred in task.predecessors:
                earliest_start = max(earliest_start, task_times[pred.name]['end'])
            
            # Find actual start time considering resource constraints
        ...
        # Find the maximum end time (makespan)
...
aco = AntColonyRCPSP(jobs, n_ants=2, n_iterations=100, alpha=1.0, beta=3.0, rho=0.1)
